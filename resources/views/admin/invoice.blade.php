...
<div class="container">
    ...
    <table>
        ...
        <tr>
            <td colspan="2" style="text-align: right;">
                Итого:&nbsp;{{ number_format($product->price_opt * $order->count, 0, ',', ' ') }}
            </td>
        </tr>
    ...
    <div class="below">
    <p>Всего наименований 1,  на сумму {{ number_format($product->price_opt * $order->count, 0, ',', ' ') }} Руб</p>
    @php
        $sum = $product->price_opt * $order->count * 100;
    @endphp
    <p>({{ mb_strtoupper(mb_substr($ntw->getCurrencyTransformer('ru')->toWords($sum, 'RUB'), 0, 1)) . mb_substr($ntw->getCurrencyTransformer('ru')->toWords($sum, 'RUB'), 1) }} 00 копеек)</p>
    </div>
    <hr class="hr">
    <div class="sign">
